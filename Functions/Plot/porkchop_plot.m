...
for departureTime = mjd_Departure
    ...
    for arrivalTime = mjd_Arrival
        ...
        [ ~, ~, ~, ~, v_i_t, v_f_t, ~, ~ ] = lambertMR( r_i, r_f, deltaT, mu_sun, 0, 0, 0, 1 ); % Lambert problem solver

        dV_t(k,j) = norm( v_i_t' - v_i ) + norm( v_f - v_f_t' ); % Total deltaV required for transfer
...
switch C3_launcher
    ...
    case inf % Unconstrained optimisation
        ...
        fprintf('\nUsing unconstrained optimisation with genetic algorith\n')
        ...
        NVARS = 3;
        x_optimal = ga(@(x) lambert_minimization_FUN( x, mu_sun, planet1, planet2 ), NVARS,A,B,Aeq,Beq,LB,UB, [], OPTIONS);
...
OPTIONS = optimoptions( 'ga', 'functiontolerance', 1e-16, 'constrainttolerance', 1e-16, 'populationsize', 200 );
        ...
        NVARS = 3;
        x_optimal = ga(@(x) lambert_minimization_FUN( x, mu_sun, planet1, planet2 ), NVARS,A,B,Aeq,Beq,LB,UB, @(x) NONLCON( x, C3_launcher, mu_sun, planet1, planet2 ), OPTIONS);
...
[ ~, dV1 ] = lambert_minimization_FUN( x_optimal, mu_sun, planet1, planet2 );
...
function [ FUN, dV1 ] = lambert_minimization_FUN( x, mu_sun, planet1, planet2 )
...
FUN = dV1 + dV2; % Total deltaV required for transfer
...
[ ~, dV1 ] = lambert_minimization_FUN( x, mu_sun, planet1, planet2 );
