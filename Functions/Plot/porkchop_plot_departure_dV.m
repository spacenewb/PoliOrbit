function [ dV_min, dV1, departureDate_minDeltaV, arrivalDate_minDeltaV, a_minDeltaV,...
    ...
    porkchop_plot_departure_dV( planet1, planet2, dateDepartureEarliest, dateDepartureLatest, ...
...
%   CONSIDERING ONLY THE INITIAL dV GIVEN AT THE FIRST PLANET
%
...
for departureTime = mjd_Departure
    ...
    for arrivalTime = mjd_Arrival
        ...
        [ ~, ~, ~, ~, v_i_t, ~, ~, ~ ] = lambertMR( r_i, r_f, deltaT, mu_sun, 0, 0, 0, 1 ); % Lambert problem solver

        dV_t(k,j) = norm( v_i_t' - v_i ); % dV required for moving onto Lambert arc from first planet
...
switch C3_launcher
    ...
    case inf % Unconstrained optimisation
        ...
        NVARS = 3;
        x_optimal = ga(@(x) lambert_minimization_FUN_departure_dV( x, mu_sun, planet1, planet2 ), NVARS,A,B,Aeq,Beq,LB,UB, [], OPTIONS);
    ...
    otherwise % Constrained optimisation
        ...
        fprintf('\nUsing constrained optimisation with genetic algorithm\n')
...
OPTIONS = optimoptions( 'ga', 'functiontolerance', 1e-16, 'constrainttolerance', 1e-16, 'populationsize', 200 );
        ...
        NVARS = 3;
        x_optimal = ga(@(x) lambert_minimization_FUN_departure_dV( x, mu_sun, planet1, planet2 ), NVARS,A,B,Aeq,Beq,LB,UB, @(x) NONLCON( x, C3_launcher, mu_sun, planet1, planet2 ), OPTIONS);
...
[ ~, dV1 ] = lambert_minimization_FUN_departure_dV( x_optimal, mu_sun, planet1, planet2 );
...
function [ FUN, dV1 ] = lambert_minimization_FUN_departure_dV( x, mu_sun, planet1, planet2 )
...
[ ~, ~, ~, ~, v_i_t, ~, ~, ~ ] = lambertMR( r_i, r_f, deltaT, mu_sun, 0, 0, 0, 1 );

dV1 = norm( v_i_t' - v_i );

FUN = dV1; % Total deltaV required for transfer
...
[ ~, dV1 ] = lambert_minimization_FUN_departure_dV( x, mu_sun, planet1, planet2 );
